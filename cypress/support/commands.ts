...
Cypress.Commands.add('clearMock', () => {
    // Allow all requests to proceed without mocking
    cy.intercept({ method: /.*/, url: /.*/ }, (req) => {
        req.continue(); // Pass the request through to the actual endpoint
    });
});

Cypress.Commands.add('mockGlobalFeeds', () => {
        cy.fixture('globalFeeds/feeds').as('feeds')
        cy.intercept('GET', '/api/articles?limit=10&offset=0', '@feeds').as('globalFeed');
})

Cypress.Commands.add('mockTags', () => {
        cy.fixture('tags/tags').as('tag')
        cy.intercept('GET', '/api/tags', '@tag').as('tags');
})
